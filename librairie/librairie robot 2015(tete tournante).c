...
void avancerCouleur();
void tournerTeteDroite(int tour); // 120 quart de tour 240 demi tour
void tournerTeteGauche(int tour);
void reculerContact(int tour);


task main()
{
	reculerContact(30000);
}

void reculerContact(int tour)//fonction avancer
{
    int valeurCapteurContactUn, valeurCapteurContactDeux;
    ...
    valeurCapteurContactUn = SensorValue(capteurContactUn);
    valeurCapteurContactDeux = SensorValue(capteurContactDeux);

    while(nMotorRunState[motorA] != runStateIdle && valeurCapteurContactUn == 0 && valeurCapteurContactDeux == 0)//Tant qu'on  pas atteint la cible ou que le bouton n'a pas été enfoncé
    {
        valeurCapteurContactUn = SensorValue(capteurContactUn);// On est obligé de relire la valeur du capteur en continue pour que la condition de la boucle s'effectue.
        valeurCapteurContactDeux = SensorValue(capteurContactDeux);
    }
    motor[motorA] = 0;//Une fois la cible atteinte on remet le moteur a 0
}

void tournerTeteDroite(int tour)//fonction avancer
{
    nMotorEncoder[motorC] = 0; //On reinitialise l'encodage du moteur
    nMotorEncoderTarget[motorC] = tour; //on definit la valeur envoye en parametre comme la cible  atteindre
    motor[motorC] = 20; //On met la puissance du moteur a 60%

    while(nMotorRunState[motorC] != runStateIdle)//Tant qu'on  pas atteint la cible on laisse le moteur  60%
    {}
    motor[motorC] = 0;//Une fois la cible atteinte on remet le moteur a 0
}

void tournerTeteGauche(int tour)//fonction avancer
{
    nMotorEncoder[motorC] = 0; //On reinitialise l'encodage du moteur
    nMotorEncoderTarget[motorC] = tour; //on definit la valeur envoye en parametre comme la cible  atteindre
    motor[motorC] = -20; //On met la puissance du moteur a 60%
...
void avancerContact(int tour)//fonction avancer
{
    ...
    valeurCapteurContact = SensorValue(capteurContactUn);
    ...
    {
        valeurCapteurContact = SensorValue(capteurContactUn);// On est obligé de relire la valeur du capteur en continue pour que la condition de la boucle s'effectue.
